  ...
  cont.innerHTML = [...grupos.entries()].map(([bloque, acts]) => {
    ...
    return `<details class="p-grupo-bloque" data-bloque="${esc(bloque)}"${abierto}>
      <summary class="p-grupo-resumen">
        ...
        <span class="p-grupo-stats">Prom. ${Math.round(acts.reduce((s,a)=>s+(a.avance||0),0)/n)}% · ${acts.filter(a=>(a.avance||0)>0).length} con avance, ${acts.filter(a=>!(a.avance||0)).length} sin</span>
      </summary>
